#----------------------------------------
### BUILD THE SIMULATION EXPERIMENT 4 ###
#----------------------------------------
...
# ---efine the spatial signal-to-noise ratios 
Ratios <- matrix(0,3,3)
Ratios[1,] <- c(0, 3, 1)
Ratios[2,] <- c(1, 0, 3)
Ratios[3,] <- c(3, 1, 0)
...
# ---define the noise covariance parameters
lambda1 <- lambda2 <- sqrt(.5)
Tau_b <- c_Delta/2
Deltab <- Tau_b*exp(-Dist^2/(2*KernelParameters$Gaussian.noise^2))+diag(rep(c_Delta-Tau_b,600))
...
for(j in 1:n_replicas){
  ...
  Sigmab <- rWishart(n = 1, df = n1+n2+n3, Sigma = diag(0.015, n1+n2+n3))[,,1]
  Xb <- t(chol(Sigmab)) %*% matrix(rnorm(600*600), 600, 600) %*% chol(Deltab)
  colnames(Xb) <- NULL
  
  # ---generate the final dataset
  x <- lambda1*Xs + lambda2*Xb
  ...
  Simulation <- list(x = x,
                     ...
                     Sigma = Sigma,
                     Sigmab = Sigmab)
  save(Simulation, file = paste("~/Scenario4_",j,".Rdata",sep=""))
}
