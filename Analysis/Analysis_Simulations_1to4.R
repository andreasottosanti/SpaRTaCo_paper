...
CER <- function(P, Q){
  if(length(P) != length(Q)) stop("Vectors have different length")
  n <- length(P)
  value <- 0
  for(i in 1:(n-1)){
    for(j in (i+1):n){
      value <- value +
        abs(as.numeric(P[i] == P[j]) - as.numeric(Q[i] == Q[j]))
    }
  }
  value/(n*(n-1)/2)
}
...
for(i in 1:n_replicas){
  ...
  for(j in 1:length(file.list)){
    ...
    if(max(results$logL) > loglikelihoods[i]){
      ...
      Best.Ds[,i] <- results$Ds
      }
  }
  CER.values[i,1] <- CER(P = Simulation$original.Cs, Q = Best.Cs[,i])
  ...
  CER.sparseBC[i,1,1] <- CER(P = Simulation$original.Cs, Q = sparseBC1$Cs)
  CER.sparseBC[i,2,1] <- CER(P = Simulation$original.Ds, Q = sparseBC1$Ds)
  CER.sparseBC[i,1,2] <- CER(P = Simulation$original.Cs, Q = sparseBC2$Cs)
  CER.sparseBC[i,2,2] <- CER(P = Simulation$original.Ds, Q = sparseBC2$Ds)
  CER.sparseBC[i,1,3] <- CER(P = Simulation$original.Cs, Q = sparseBC3$Cs)
  ...
  CER.MVNb[i,2,2] <- CER(P = Simulation$original.Ds, Q = MVNb2$Ds)
  CER.MVNb[i,1,3] <- CER(P = Simulation$original.Cs, Q = MVNb3$Cs)
  CER.MVNb[i,2,3] <- CER(P = Simulation$original.Ds, Q = MVNb3$Ds)
